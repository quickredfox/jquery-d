(function() {
  /*
      ...
      Dastardly
          ...
          $.when( fetchTwitterProfile, fetchTwitterTimeline ).done( function( value ){
              // I forget, what's "value" ? 
          });
  ...
  */  var $, D, Negociator, coerce, fargs, jQuery, parallel, pushError, pushValue, sequence;
  ...
  */
  D.isPromise = function(value) {
    return Boolean(value && value.promise && 'function' === typeof value.promise && value.promise().promise);
  };
  /*
      D.isDeferred( value )
      --------------------------------------------------
  ...
  */
  D.deep = function(values) {
    ...
    operation = D();
    todo = [];
    if ($.isPlainObject(values) || $.type(values) === 'array') {
      $.each(values, function(key, value) {
        ...
        } else if ($.isPlainObject(values) || $.type(values) === 'array') {
